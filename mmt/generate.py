...
def main():
    ...
    device = torch.device(f"cuda:{args.gpu}" if args.gpu is not None else "mps")
    ...
    with torch.no_grad():
        ...
        for i in tqdm.tqdm(range(args.n_samples), ncols=80):
            ...
            # Split the array
            first_part = batch["seq_in"].to(device)
            second_part = batch["seq_out"].to(device)
            name = batch["name"][0]

            # ------------
            # Ground truth
            # ------------
            truth_np = second_part[0].cpu().numpy()
            save_result(f"{name}_truth", truth_np, sample_dir, encoding)
            ...
            # Save the results
            save_result(f"{name}_translate", generated_np[0], sample_dir, encoding)
