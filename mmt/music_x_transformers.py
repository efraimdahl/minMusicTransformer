    ...
    def generate(
        ...
        **kwargs,
        ...
        if monotonicity_dim is not None:
            current_values = {
                d: torch.max(input_seq[:, :, d], 1)[0] for d in monotonicity_dim
            }
        else:
            current_values = None
        ...
        logits_full = self.net(input_seq, **kwargs)

        for i in range(logits_full[0].shape[1]):
            logits = [l[:, i, :] for l in logits_full]
        ...
        for batch in range(out.shape[0]):
            # Ensure they are the instrument.
            items_with_first_feature_3 = out[batch, out[batch, :, 0] == 3]

            # Find the most common last feature among these items
            if len(items_with_first_feature_3) > 0:
                last_features = items_with_first_feature_3[:, -1].tolist()
                most_common_last_feature = Counter(last_features).most_common(1)[0][0]

                # Set the last feature of all items that start with 3 to this value
                out[batch, out[batch, :, 0] == 3, -1] = most_common_last_feature
        return out

    def forward(self, x, y, return_list=False, **kwargs):
...
class MusicXTransformer(nn.Module):
    ...
    def forward(self, seq_in, seq_out, mask=None, **kwargs):
        return self.decoder(seq_in, seq_out, mask=mask, **kwargs)
