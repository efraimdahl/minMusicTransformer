...
def parse_args(args=None, namespace=None):
    ...
    parser.add_argument(
        "--transfer_dir", type=pathlib.Path, help="transfer model directory"
    )
...
def main():
    ...
    if args.transfer_dir:
        checkpoint_dir = args.transfer_dir / "checkpoints"
    else:
    ...
    while step < args.steps:
        ...
        for batch in (pbar := tqdm.tqdm(range(args.valid_steps), ncols=80)):
            ...
            seq_out = batch["seq_out"].to(device)

            # Update the model parameters
            optimizer.zero_grad()
            loss = model(seq_in, seq_out)
            loss.backward()
        ...
        with torch.no_grad():
            ...
            for batch in valid_loader:
                ...
                count += len(batch)
                total_loss += len(batch) * float(loss.item())
