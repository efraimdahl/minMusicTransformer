...
def parse_args(args=None, namespace=None):
    ...
    parser = argparse.ArgumentParser(description="Extract note sequences from music")
    parser.add_argument(
        "-d",
        "--dataset",
    ...
    parser.add_argument(
        "-o",
        "--out_dir",
        default="data/brq/processed/notes/",
        type=pathlib.Path,
...
def main():
    ...
    if args.dataset is not None:
        ...
        if args.out_dir is None:
            args.out_dir = pathlib.Path(f"data/{args.dataset}/processed/notes/")
    ...
    logging.info(f"Extracted {len(extracted_names)} out of {len(names)} files.")
